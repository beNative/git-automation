...
const DirtyRepoModal: React.FC<DirtyRepoModalProps> = ({ isOpen, status, onChoose, isIgnoring }) => {
  ...
  const handleIgnoreClick = () => {
    onChoose('ignore', Array.from(selectedFiles));
  };
  
  const hasUntracked = status.untrackedFiles.length > 0;
            ...
            <div className="ml-4 text-left flex-1">
              ...
              <div className="mt-2">
                <p className="text-sm text-gray-500 dark:text-gray-400">
                  The repository has local changes. Choose how to proceed before running the task.
                </p>
              </div>
            </div>
          </div>
